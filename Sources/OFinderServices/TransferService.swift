...
public enum TransferService {
    ...
    private static func start(rsyncPath: String, sources: [String], dstDir: String,
        ...
        Thread.detachNewThread {
            run(rsyncPath: rsyncPath, sources: sources, dstDir: dstDir,
                overwrite: overwrite, isMove: isMove, handle: handle,
    ...
    private static func run(rsyncPath: String, sources: [String], dstDir: String,
                            overwrite: Bool, isMove: Bool, handle: TransferHandle,
                            onProgress: @escaping ProgressHandler,
        ...
        // A cancelled job exits non-zero, but that is not an error to report —
        // and the source cleanup below must not run, since only part of the
        // set made it across.
