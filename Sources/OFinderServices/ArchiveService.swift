...
public enum ArchiveService {
    ...
    public static func compress(sevenzzPath: String, sources: [String],
                                ...
                                onDone: @escaping CompletionHandler) -> TransferHandle {
        ...
        queue.async {
            ...
            run(sevenzzPath: sevenzzPath, arguments: args,
                ...
                handle: handle, onProgress: onProgress, onDone: onDone)
        }
        return handle
    }
    ...
    private static func run(sevenzzPath: String, arguments: [String],
        ...
        // Cancelled: the non-zero exit is what the user asked for, not a
        // failure worth an error panel. A half-written archive is left on
        // disk — 7zz appends to an existing one, so deleting it here could
        // destroy an archive that was already there.
