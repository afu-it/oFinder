...
enum FavoritesStore {
    ...
    /// The bundle identifier this app shipped under before the rename.
    ///
    /// Preferences are filed by bundle identifier, so changing it left the
    /// old favorites sitting in a domain nothing reads any more. This copies
    /// them across once.
    private static let previousBundleID = "com.example.r2finder"

    /// Adopts favorites saved under the old bundle identifier.
    ///
    /// Runs only when this app has nothing of its own: the old domain is a
    /// starting point, never an override, so a later launch cannot resurrect
    /// a layout the user has since changed. Deliberately does not delete the
    /// old domain — if the migration is ever wrong, the original is still
    /// there to look at.
    private static func adoptPreviousFavoritesIfNeeded() {
        guard UserDefaults.standard.array(forKey: defaultsKey) == nil,
              let previous = UserDefaults(suiteName: previousBundleID),
              let order = previous.array(forKey: defaultsKey) as? [String]
        else { return }

        UserDefaults.standard.set(order, forKey: defaultsKey)
        if let removed = previous.array(forKey: removedKey) as? [String] {
    ...
    static func entries() -> [Entry] {
        adoptPreviousFavoritesIfNeeded()
